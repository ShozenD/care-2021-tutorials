...
NHIS is that columns have really obscure column names. For example, the
column that contains the race/ethnicity info is called **MRACBPI2**
(Don't ask me why). So, one the first things you'll want to do is change
the column name.
...
ball. Here, `%>%` passes the contents of `df.nhis.2` the mutate function. Note that we no longer have to include `df.nhis.2` in the first argument of `mutate()`. The pipe is a powerful tool because it allows you to focus on the transformations, not what's being transformed, which make the code easier to read. It will help you chain a series data transformation operations to create a data wrangling "sentence". 
...
example where we count the number of repondents in each race/ethnicity
group.
